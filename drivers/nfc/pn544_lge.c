	...
	if (!stReadIntFlag) {
		...
		if (irq_gpio_val == 0) {
			if (filp->f_flags & O_NONBLOCK) {
				dprintk(PN544_DRV_NAME ":f_falg has O_NONBLOCK. EAGAIN!\n");
				ret = -EAGAIN;
				goto fail;
			}
		...
		size_t count, loff_t *offset)
{
	...
	if (copy_from_user(tmp, buf, count)) {
		dprintk(PN544_DRV_NAME ":%s : failed to copy from user space\n", __func__);
		return -EFAULT;
	}
...
//                                        
err_exit:
	dprintk(PN544_DRV_NAME ": pn544_dev is null\n");

err_firm:
	dprintk(PN544_DRV_NAME ": nfc_firm request failed\n");
//	gpio_free(pn544_dev->ven_gpio);

err_ven:
	dprintk(PN544_DRV_NAME ": nfc_ven request failed!\n");
//	gpio_free(pn544_dev->firm_gpio);

	dprintk(PN544_DRV_NAME ": pn544_probe() end with error!\n");
