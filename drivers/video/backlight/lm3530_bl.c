...
static int requested_in_early_suspend_lcd_level= MIN_LEVEL;
#endif  
#endif
...
void lm3530_backlight_on(int level)
{
		...
		{
			printk("%s: requested_in_early_suspend_lcd_level: %d)\n",
				__func__, level);
			return;
		}
...
#ifdef CONFIG_FB_MSM_MIPI_LGD_VIDEO_QHD_PT_PANEL
	msleep(1);
#endif
...
void lm3530_late_resume(struct early_suspend * h)
{
	...
	/*(1) lm3530_lcd_backlight_set_level(requested_in_early_suspend_lcd_level);								*/
	/*	  It's in BL_OFF, do backlight hw reset. it's possible task switching on hw resettingby msm_fb_set_backlight .	*/
	/*(2) lm3530_lcd_backlight_set_level(requested_in_early_suspend_lcd_level);	 							*/
	/*	  . It's in BL_ON, skip backlight hw reset.															*/
	lm3530_lcd_backlight_set_level(requested_in_early_suspend_lcd_level);
	return;
}
#endif /* CONFIG_HAS_EARLYSUSPEND */
